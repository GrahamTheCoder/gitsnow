...
from .dependencies import get_all_dependency_information, get_dependency_ordered_objects, order_objects_topologically
...
@click.option('--allow-data-loss', is_flag=True, help="Allow dropping tables that may contain data.")
@click.option('--test', is_flag=True, help="Use a mock connection for testing.")
@click.pass_context
def folder_to_script(ctx, db_name, output_file_prefix, allow_data_loss, test):
    ...
    try:
        ...
        source_obj_names = {obj[0].upper() for obj in ordered_obj_paths}
        db_obj_names = set(db_objects.keys())
        deleted_obj_names = db_obj_names - source_obj_names

        deleted_dependencies = {}
        if deleted_obj_names:
            click.echo(f"Found {len(deleted_obj_names)} deleted objects to drop.")
            for obj_name in deleted_obj_names:
                ddl = db_objects[obj_name].ddl
                lineage = LineageRunner(sql=ddl, dialect="snowflake", silent_mode=True)
                dependencies = {dep.raw_name.upper() for dep in lineage.source_tables}
                deleted_dependencies[obj_name] = dependencies

        ordered_deleted_objects = order_objects_topologically(list(deleted_obj_names), deleted_dependencies)
        ...
        with open(up_script_path, 'w', encoding='utf-8') as f_up:
            ...
            if ordered_deleted_objects:
                f_up.write("-- The following objects will be dropped:\n")
                for obj_name in reversed(ordered_deleted_objects):
                    f_up.write(f"--   {obj_name}\n")
                f_up.write("\n")
            ...
            if ordered_deleted_objects:
                f_up.write("-- Drop deleted objects\n")
                for obj_name in reversed(ordered_deleted_objects):
                    obj_type = db_objects[obj_name].type
                    drop_statement = f"DROP {obj_type} IF EXISTS {obj_name};\n"
                    if obj_type in ["TABLE", "DYNAMIC TABLE"] and not allow_data_loss:
                        f_up.write(f"-- {drop_statement}")
                        f_up.write(f"-- UNCOMMENT PREVIOUS LINE TO DROP TABLE/DYNAMIC TABLE (requires --allow-data-loss flag)\n\n")
                    else:
                        f_up.write(drop_statement + "\n")
